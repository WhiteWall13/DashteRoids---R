...
server <- function(input, output) {
  ...
  output$line_chart <- renderPlotly({
    filtered_data <- data[data$year_numeric >= input$year_range[1] & data$year_numeric <= input$year_range[2], ]
  ...
  output$map_chart <- renderPlotly({
    # Utilisez votre GeoDataFrame à la place de "VOTRE_GEODATAFRAME"
    gdf <- data
    
    # Appeler la fonction pour créer le graphique
    map <- draw_scatter_mapbox(gdf, color = "ylorrd_r", layer = "basic")
